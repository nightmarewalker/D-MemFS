# MemoryFileSystem (MFS) 詳細実装設計書 v2 (DetailedDesignSpec_v2)

本書は `spec_v14.md` を実装者向けに具体化した詳細設計書である。クラス定義・アルゴリズム・擬似コード・エラーハンドリング網羅表を含み、これ単体で実装に着手可能なレベルの情報を提供する。

> **ドキュメント位置づけ（重要）**: 本書には設計意図を残すための履歴的な擬似コード・解説が含まれる。実装との相違が生じた場合は、`spec_v14.md` と実コード（`dmemfs/*.py`）を正とする。
...
> **v14 更新**: 物理メモリ保護 `MemoryGuard` を追加。`_memory_info.py` / `_memory_guard.py` の新規内部モジュール、`MemoryFileSystem` / `AsyncMemoryFileSystem` の `memory_guard` 系パラメータ、`write_at` / `truncate(拡張)` / `import_tree()` / `copy_tree()` / `preallocate` への統合、`MemoryError` 説明メッセージの改善を追記した。
...
├── _memory_info.py           # OS依存の利用可能メモリ検出（内部）
├── _memory_guard.py          # MemoryGuard 戦略層（内部）
├── _file.py                  # IMemoryFile, SequentialMemoryFile, RandomAccessMemoryFile
├── _handle.py                # MemoryFileHandle
├── _text.py                  # MFSTextHandle (テキストモードラッパー)
...
class IMemoryFile(ABC):
    ...
    @abstractmethod
    def write_at(
        self, offset: int, data: bytes, quota_mgr, memory_guard=None
    ...
    @abstractmethod
    def truncate(self, size: int, quota_mgr, memory_guard=None) -> None: ...
...
class SequentialMemoryFile(IMemoryFile):
    ...
    def write_at(self, offset: int, data: bytes, quota_mgr, memory_guard=None
    ) -> "tuple[int, RandomAccessMemoryFile | None, int]":
    ...
    def _promote_and_write(self, offset: int, data: bytes, quota_mgr, memory_guard=None
...
class MemoryFileHandle(io.RawIOBase):
    ...
    def write(self, data: bytes) -> int:
        ...
        n, promoted, old_quota = self._fnode.storage.write_at(
            self._cursor, data, self._mfs._quota, self._mfs._memory_guard
        )
    ...
    # ── flush / capability queries ────────────────────────────────
    def flush(self) -> None:
        self._assert_open()
        return None  # メモリ上のため no-op
...
class MemoryFileSystem:
    def __init__(
        ...
        self._promotion_hard_limit = promotion_hard_limit
...
> **v14 設計判断**: `MemoryGuard` は `QuotaManager` に注入しない。論理クォータ管理と物理メモリ警戒の責務を分離するため、`_fs.py` / `_file.py` のメモリ確保直前で呼び出す。
...
## 26. [v14] 物理メモリ保護 (MemoryGuard)

### 26.1 `_memory_info.py`

`get_available_memory_bytes() -> int | None` を提供する内部モジュールを追加する。公開 API には含めない。
...
- Linux: cgroup v2 → cgroup v1 → `/proc/meminfo` を初回のみプローブし、reader 関数をキャッシュ
- macOS: `host_statistics64` を `ctypes` で呼び、`free + speculative` を利用可能量とみなす

Linux 実装では `_UNPROBED` センチネルと `_linux_reader` を用い、2 回目以降の `per_write` 問い合わせでプローブを再実行しない。

### 26.2 `_memory_guard.py`
...
- `NullGuard`: 何もしない
- `InitGuard`: `max_quota > available_memory` のとき警告または `MemoryError`
- `PerWriteGuard`: `time.monotonic()` と `_last_check` / `_last_avail` を使ってキャッシュ付きで検査

`action="warn"` は `ResourceWarning`、`action="raise"` は `MemoryError` を送出する。

### 26.3 `_file.py` への統合

`IMemoryFile.write_at()` および `truncate()` の内部呼び出しに `memory_guard` を渡せるようにし、`reserve()` の直前で `check_before_write()` を実行する。

```python
def write_at(self, offset: int, data: bytes, quota_mgr: QuotaManager,
...
def truncate(self, size: int, quota_mgr: QuotaManager,
             memory_guard: MemoryGuard | None = None):
    if size > current_size:
        extend = size - current_size
        if memory_guard is not None:
            memory_guard.check_before_write(extend)
        with quota_mgr.reserve(extend):
            ...
...
- `SequentialMemoryFile.truncate()` の拡張経路（POSIX ゼロ埋め）
- `RandomAccessMemoryFile.truncate()` の拡張経路（POSIX ゼロ埋め）
- `SequentialMemoryFile._promote_and_write()` の一時 2 倍確保経路
- `MemoryFileHandle.write()` から `write_at()` への `memory_guard` 転送
- `MemoryFileHandle.truncate()` から `truncate()` への `memory_guard` 転送

### 26.4 `_fs.py` への統合

`_force_reserve()` を使う経路では、`QuotaManager` ではなく呼び出し側で明示的にガードを呼ぶ。
...
`open(..., preallocate=N)` では、ゼロ埋め書き込みの直前に `preallocate - current` をガードへ渡す。

### 26.5 `MemoryError` メッセージ改善

実メモリ確保に失敗した場合は、元の `MemoryError` をそのまま露出させず、文脈付きメッセージへ変換する。
...
- `RandomAccessMemoryFile.write_at()`
- 各 `truncate()` の拡張経路
- `SequentialMemoryFile._promote_and_write()`
- `MemoryFileSystem.import_tree()` 内の `_bulk_load()` 呼び出し

### 26.6 `_async.py` への統合

`AsyncMemoryFileSystem.__init__()` は新規 3 パラメータを同期版へそのまま転送する。

```python
class AsyncMemoryFileSystem:
    def __init__(..., memory_guard: str = "none",
                 memory_guard_action: str = "warn",
...
### 26.7 v14 対応確認チェックリスト

| spec_v14.md の項目 | 本書の対応箇所 | 実装状態 |
|---|---|---|
| `_memory_info.py` 新設 | §26.1 | 設計追記 |
| `MemoryGuard` ストラテジー層 | §26.2 | 設計追記 |
| `MemoryFileSystem.__init__` の新規パラメータ | §10.1, §26.4 | 設計追記 |
| `write_at` / `truncate` / `preallocate` への統合 | §10.4, §26.3 | 設計追記 |
| `import_tree()` / `copy_tree()` への統合 | §26.4 | 設計追記 |
| `MemoryError` メッセージ改善 | §26.5 | 設計追記 |
| `AsyncMemoryFileSystem` への引き継ぎ | §26.6 | 設計追記 |

---

## 付録A：v9 からの変更サマリ

| 項目 | v9 | v10 | v11 | v12 | v14 | 変更根拠 |
|---|---|---|---|---|---|---|
| 内部ストレージ構造 | `_tree: dict[str, IMemoryFile]` | `DirNode/FileNode` + `_nodes: dict[int, Node]` | 同左 | 同左 | 同左 | listdir $O(N)$ → $O(\text{children})$ 改善 |
| ロック階層 | 2層（global → rw） | 3層（global → quota → rw） | 同左 | 同左 | 同左 | ディレクトリインデックス層との親和性 |
| `wb` truncate 順序 | ロック取得**前** | ロック取得**後** | 同左 | 同左 | 同左 | PEP 703 対応 |
| `walk()` 整合性 | 暗黙的 | 弱整合性を**明記** | 同左 | `list()` スナップショット追加 | 同左 | GILフリー安全性 |
| `glob()` パターン | `fnmatch.fnmatch` | `*` は `/` 以外、`**` で再帰マッチ | 同左 | 同左 | 同左 | 標準glob挙動との整合 |
| `export_as_bytesio()` | `_global_lock` なし | `_global_lock` で保護 | 同左 | 同左 | 同左 | GILフリー対応 |
| `__del__` stacklevel | `stacklevel=2` | `stacklevel=1` | 同左 | 同左 | 同左 | GCから呼ばれるため |
| `stats()` chunk_count | 暗黙的 | SequentialMemoryFile のみと**明記** | 同左 | 同左 | 同左 | ドキュメント品質向上 |
| 新規API (v10) | — | `copy_tree()`, `move()`, `glob("**")` | 同左 | 同左 | 同左 | ツリー操作の充実 |
| `is_dir`等の所在 | `IMemoryFile` 内 | `DirNode`/`FileNode` に分離 | 同左 | 同左 | 同左 | 関心の分離 |
| **タイムスタンプ** | なし | ロードマップのみ | `FileNode` に追加、`stat()` API | 同左 | 同左 | 差分処理判定、アーカイブ対応 |
| **bytearray shrink** | なし | ロードマップのみ | `truncate()` に shrink 機構 | 同左 | 同左 | メモリ効率の改善 |
| **PEP 703 対応** | なし | ロードマップのみ | 対応設計の詳細化 | 同左 | 同左 | free-threaded Python への備え |
| **async/await** | なし | ロードマップのみ | `AsyncMemoryFileSystem` ラッパー | 同左 | 同左 | asyncio アプリとの統合 |
| **`_NoOpQuotaManager`** | — | — | — | **削除** | 同左 | 未使用コード除去 |
| **`AsyncMemoryFileSystem` 公開** | なし | なし | `__getattr__` 遅延インポート | 同左（遅延インポート確認） | 同左 | `asyncio` コスト回避 |
| **`copy()` API仕様** | — | — | — | **引数仕様・例外仕様を明記** | 同左 | ドキュメント品質 |
| **`_force_reserve()` 制約** | — | — | — | **使用条件を明記** | 同左 | 安全性・保守性 |
| **物理メモリ保護** | なし | なし | なし | なし | **`MemoryGuard` / `_memory_info.py` / メモリ不足の事前警戒** | 論理クォータと OS 物理メモリの乖離を補完 |
